...
private slots:
    ...
    // LD (BC), A
    void test_ld_addr_bc_a();

    // LD (DE), A
    void test_ld_addr_de_a();

    // LD (nn), HL
    void test_ld_addr_imm_hl();

    // LD (nn), A
    void test_ld_addr_imm_a();

    // LD A, (BC)
    void test_ld_a_addr_bc();

    // LD A, (DE)
    void test_ld_a_addr_de();

    // LD A, (nn)
    void test_ld_a_addr_imm();

    // LD HL, (nn)
    void test_ld_hl_addr_imm();

    // INC rr
    void test_inc_rr_data();
    void test_inc_rr();
    void test_inc_sp();

    // DEC rr
    void test_dec_rr_data();
    void test_dec_rr();
    void test_dec_sp();
};
...
void Bz80BaseCpuInstructionsTest::test_ld_addr_bc_a() {
    const uint8_t opcode = (uint8_t)0x02;
    const uint8_t startingAValue = (uint8_t)0xac;
    const uint16_t address = (uint16_t)0x02;
    const FlagRegister startingFlags = genRandomFlags();
    const uint8_t expCycles = MEMORY_ACCESS_CYCLES;

    this->cpu->registerA = startingAValue;
    this->cpu->registerBC.set16(address);
    this->cpu->registerF = startingFlags;
    ...
    QCOMPARE(this->bus.read8(address, false), startingAValue);
    QCOMPARE(this->cpu->registerF, startingFlags);
    QCOMPARE(cycles, expCycles);
}

void Bz80BaseCpuInstructionsTest::test_ld_addr_de_a() {
    const uint8_t opcode = (uint8_t)0x12;
    const uint8_t startingAValue = (uint8_t)0x40;
    const uint16_t address = (uint16_t)0x9;
    const FlagRegister startingFlags = genRandomFlags();
    const uint8_t expCycles = MEMORY_ACCESS_CYCLES;

    this->cpu->registerA = startingAValue;
    this->cpu->registerDE.set16(address);
    this->cpu->registerF = startingFlags;
...
void Bz80BaseCpuInstructionsTest::test_ld_addr_imm_hl() {
    const uint8_t opcode = (uint8_t)0x22;
    const uint16_t startingHLValue = (uint16_t)0xb;
    const uint16_t address = (uint16_t)0x8;
    const FlagRegister startingFlags = genRandomFlags();
    const uint8_t expCycles = MEMORY_ACCESS_CYCLES*4;

    this->bus.write16(0x1, address);
    ...
    QCOMPARE(this->bus.read16(address), startingHLValue);
    QCOMPARE(this->cpu->registerF, startingFlags);
    QCOMPARE(cycles, expCycles);
}

void Bz80BaseCpuInstructionsTest::test_ld_addr_imm_a() {
    const uint8_t opcode = (uint8_t)0x32;
    const uint8_t startingAValue = (uint8_t)0x63;
    const uint16_t address = (uint16_t)0x03;
    const FlagRegister startingFlags = genRandomFlags();
    const uint8_t expCycles = MEMORY_ACCESS_CYCLES*3;
...
void Bz80BaseCpuInstructionsTest::test_ld_a_addr_bc() {
    const uint8_t opcode = (uint8_t)0x0a;
    const uint8_t memoryValue = (uint8_t)0xb;
    const uint8_t address = (uint16_t)0x1;
    const FlagRegister startingFlags = genRandomFlags();
    const uint8_t expCycles = MEMORY_ACCESS_CYCLES;

    this->bus.write8(address, memoryValue, false);
    ...
    QCOMPARE(this->cpu->registerA, memoryValue);
    QCOMPARE(this->cpu->registerF, startingFlags);
    QCOMPARE(cycles, expCycles);
}

void Bz80BaseCpuInstructionsTest::test_ld_a_addr_de() {
    const uint8_t opcode = (uint8_t)0x1a;
    const uint8_t memoryValue = (uint8_t)0x67;
    const uint8_t address = (uint16_t)0x6;
    const FlagRegister startingFlags = genRandomFlags();
    const uint8_t expCycles = MEMORY_ACCESS_CYCLES;
...
void Bz80BaseCpuInstructionsTest::test_ld_a_addr_imm() {
    const uint8_t opcode = (uint8_t)0x3a;
    const uint8_t memoryValue = (uint8_t)0xb0;
    const uint8_t address = (uint16_t)0x4;
    const FlagRegister startingFlags = genRandomFlags();
    const uint8_t expCycles = MEMORY_ACCESS_CYCLES*3;

    this->bus.write16(1, address);
    this->bus.write8(address, memoryValue, false);
...
void Bz80BaseCpuInstructionsTest::test_ld_hl_addr_imm() {
    const uint8_t opcode = (uint8_t)0x2a;
    const uint8_t memoryValue = (uint8_t)0xe9e5;
    const uint8_t address = (uint16_t)0x3;
    const FlagRegister startingFlags = genRandomFlags();
    const uint8_t expCycles = MEMORY_ACCESS_CYCLES*4;

    this->bus.write16(1, address);
    this->bus.write16(address, memoryValue);
    ...
    QCOMPARE(this->cpu->registerHL.get16(), memoryValue);
    QCOMPARE(this->cpu->registerF, startingFlags);
    QCOMPARE(cycles, expCycles);
}

void Bz80BaseCpuInstructionsTest::test_inc_rr_data() {
    QTest::addColumn<uint8_t>("opcode");
    QTest::addColumn<uint16_t>("startingValue");
    QTest::addColumn<RegisterPairType*>("registerPair");
    QTest::addColumn<FlagRegister>("startingFlags");
    ...
    QTest::addRow("INC BC")
        << (uint8_t)0x03 << (uint16_t)0xd7e5 << &this->cpu->registerBC
        << genRandomFlags();

    QTest::addRow("INC DE")
        << (uint8_t)0x13 << (uint16_t)0x1261 << &this->cpu->registerDE
        << genRandomFlags();

    QTest::addRow("INC HL")
        << (uint8_t)0x23 << (uint16_t)0x313d << &this->cpu->registerHL
        << genRandomFlags();
}

void Bz80BaseCpuInstructionsTest::test_inc_rr() {
    QFETCH(uint8_t, opcode);
    QFETCH(uint16_t, startingValue);
    QFETCH(RegisterPairType*, registerPair);
    QFETCH(FlagRegister, startingFlags);

    const uint16_t expValue = startingValue + 1;
    const uint8_t expCycles = 6;

    registerPair->set16(startingValue);
    this->cpu->registerF = startingFlags;
    ...
    QCOMPARE(registerPair->get16(), expValue);
    QCOMPARE(cycles, expCycles);
    QCOMPARE(this->cpu->registerF, startingFlags);
}

void Bz80BaseCpuInstructionsTest::test_inc_sp() {
    const uint8_t opcode =  0x33;
    const uint16_t startingValue = 0x4ce3;
    const FlagRegister startingFlags = genRandomFlags();

    const uint16_t expValue = startingValue + 1;
    const uint8_t expCycles = 6;

    this->cpu->stackPointer = startingValue;
    this->cpu->registerF = startingFlags;
    ...
    QCOMPARE(this->cpu->stackPointer, expValue);
    QCOMPARE(cycles, expCycles);
    QCOMPARE(this->cpu->registerF, startingFlags);
}

void Bz80BaseCpuInstructionsTest::test_dec_rr_data() {
    QTest::addColumn<uint8_t>("opcode");
    QTest::addColumn<uint16_t>("startingValue");
    QTest::addColumn<RegisterPairType*>("registerPair");
    QTest::addColumn<FlagRegister>("startingFlags");
    ...
    QTest::addRow("DEC BC")
        << (uint8_t)0x0B << (uint16_t)0x829c << &this->cpu->registerBC
        << genRandomFlags();

    QTest::addRow("DEC DE")
        << (uint8_t)0x1B << (uint16_t)0x9263 << &this->cpu->registerDE
        << genRandomFlags();

    QTest::addRow("DEC HL")
        << (uint8_t)0x2B << (uint16_t)0x12a1 << &this->cpu->registerHL
        << genRandomFlags();
}

void Bz80BaseCpuInstructionsTest::test_dec_rr() {
    QFETCH(uint8_t, opcode);
    QFETCH(uint16_t, startingValue);
    QFETCH(RegisterPairType*, registerPair);
    QFETCH(FlagRegister, startingFlags);
...
void Bz80BaseCpuInstructionsTest::test_dec_sp() {
    const uint8_t opcode =  0x3B;
    const uint16_t startingValue = 0x6a0f;
    const FlagRegister startingFlags = genRandomFlags();
